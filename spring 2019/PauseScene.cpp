...
PauseScence::PauseScence(SceneLoader* loader, Sounds* SND)
{
    //ctor
    this->SND = SND;
    this->loader = loader;
...
PauseScence::~PauseScence()
{
    //dtor
}
GLint PauseScence::initGL()
{
...
int PauseScence::winMsg(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
//case WM_KEYDOWN:
            //if(wParam == VK_SPACE){
      ...
      switch(uMsg){
        case WM_KEYDOWN:
            if(wParam == VK_SPACE){
                   // GLScene *pause = new PauseScence(loader);
		...
		*/
		...
		case WM_LBUTTONDOWN:
        {
            ...
            if(mouseX > -4.61752  && mouseX < 4.5715  && mouseY > 1.80252  && mouseY < 3.10647){
                SND->stopAllSounds();
                loader->actuallyLoadScene("select");//or select
