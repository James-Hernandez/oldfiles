...
loadShader *shaderP = new loadShader();
...
GLint GLScene::initGL()
{
 ...
 //   snds->playMusic("sounds/MF-W-90.XM");
    ...
    crate.bindTexture("images/1.jpg");
    shader->initShader("v.vs", "f.fs");
    shaderP->initShader("v1.vs", "f1.fs"); // for particles

    objtex->bindTexture("images/fl.png");
    objtex1->bindTexture("images/fll.png");

    enm->bindTexture("images/mon.png");

    for(int i=0;i<20;i++)
        {
        (i<10)?obj[i].objectTex = objtex->tex:obj[i].objectTex = objtex1->tex;
           ...
           E[i].EnemyTex= enm->tex;
           E[i].xPos = (float)(rand()) / float(RAND_MAX)*5-2.5;
           E[i].yPos = -0.5;

           E[i].placeEnemy(E[i].xPos,E[i].yPos,-3.0);
           E[i].ySize=E[i].xSize= (float)(rand()%12)/50.0;
        }
