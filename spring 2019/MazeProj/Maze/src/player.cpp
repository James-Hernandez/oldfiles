...
if(arrowStatus){
   glColor3f(1.0,1.0,1.0);
     ...
     if(T->GetTicks()>500)//higher the number the slower the arrow/ energy wave.
        {
...
void Player::drawplayer()
{
   ...
   glTranslatef(plyLoc.x ,plyLoc.y,0.0);
    ...
    //glScaled(1.0/(float)gridSize,1.0/(float)gridSize,1);
glScaled(1.0/(float)gridSize,1.0/(float)gridSize,1);//controls size of player
...
void Player::initPlayer(int gSize,int frams, char *FileName)
{
    ...
    t= unitWidth;///stepsPerMove;//this has the player traverse smoothly accross grid
    ///if changed to t = unitWidth then player goes grid square by grid square.
...
for(int i = 0;i <numberOfWalls;i++){
    if(unitWidth < sqrt(pow((plyLoc.x - W[i].wallBrk.x),2))+ sqrt(pow((plyLoc.y - W[i].wallBrk.y),2)) ){
        return true;
    }
}
collision = false;
